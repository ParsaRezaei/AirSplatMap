...
def main():
    ...
    try:
        for frame in source:
            ...
            cv2.putText(rendered_bgr, "3DGS RENDER", (10, 30), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
            ...
            if num_gaussians > 0:
                ...
                if sh_dc.ndim == 3:
                    ...
                    colors = np.full((len(xyz), 3), 128, dtype=np.uint8)  # Gray default
            ...
            # Add label
            cv2.putText(splat_view, f"GAUSSIAN SPLATS ({num_gaussians:,})", (10, 30), 
        ...
        for i, orbit_pose in enumerate(orbit_poses):
            ...
            info_panel[:] = (30, 30, 30)
            cv2.putText(info_panel, f"MODEL FLYTHROUGH - {engine.get_num_gaussians():,} Gaussians", 
        ...
        if args.mode == 'mapping':
            ...
            for i, frame in enumerate(source):
                ...
                if i % 10 == 0:
                    ...
                    # Add labels with PSNR
                    cv2.putText(gt_bgr, f"GT (frame {frame.idx})", (10, 30), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                    cv2.putText(rendered_bgr, f"Rendered (PSNR: {psnr:.1f}dB)", (10, 30), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        ...
        if args.mode == 'mapping' and engine.get_num_gaussians() > 0:
            ...
            try:
                ...
                try:
                    ...
                    # Save mesh as PLY
                    mesh_path = output_dir / "final" / "mesh.ply"
                    ...
                    for i, orbit_pose in enumerate(orbit_poses):
                        ...
                        info_panel = np.zeros((60, width, 3), dtype=np.uint8)
                        info_panel[:] = (30, 30, 30)
                        cv2.putText(info_panel, f"MESH FLYTHROUGH - {len(verts):,} vertices, {len(faces):,} faces", 
