...
\subsection{Baseline Architecture: GraphDeco}

We use GraphDeco~\cite{kerbl3Dgaussians} as our primary baseline due to its superior quality in our experiments. The key architectural choices are:
...
\begin{itemize}
\item \texttt{fr1\_desk} (573 frames): Office desk scene with various objects
\item \texttt{fr1\_desk2} (620 frames): Similar desk with different viewpoint
\item \texttt{fr1\_room} (1,352 frames): Full room traversal
\item \texttt{fr1\_xyz} (798 frames): Simple XYZ translation motion
\item \texttt{fr2\_desk} (2,890 frames): Longer desk sequence
\item \texttt{fr2\_xyz} (3,669 frames): Extended translation sequence
\item \texttt{fr3\_long\_office} (2,585 frames): Long office traversal
\end{itemize}
...
\textbf{Ubuntu Desktop:} Intel i9-9600K (6-core @ 3.7 GHz), 64 GB RAM, NVIDIA RTX 2080 Ti (11 GB VRAM), 250W TDP, Ubuntu 22.04, CUDA 12.1.
...
\textbf{MacBook Pro:} Apple M2 Pro with 16 GB unified memory. Note: 3DGS engines require CUDA and cannot run on MPS; only pose and depth estimation methods were evaluated on this platform.
...
\caption{\textbf{3DGS Engine Quality Comparison.} PSNR achieved by each engine on TUM RGB-D with ground truth pose and depth. GraphDeco achieves the highest quality.}
...
\begin{table}[t]
\centering
\caption{\textbf{3DGS Engine Comparison} on TUM fr1\_desk with ground truth inputs. gsplat and Gaussian-SLAM achieve best quality; gsplat offers fastest training.}
...
\begin{table}[t]
\centering
\caption{\textbf{Per-Sequence PSNR (dB).} gsplat consistently outperforms on TUM; all engines improve on noise-free Replica.}
...
fr3\_office & 21.45 & 20.78 & 10.34 & 20.12 \\
\midrule
...
\begin{table}[t]
\centering
\caption{\textbf{Jetson Orin Deployment.} Training is impractical (1.87 FPS) but rendering achieves real-time (38.7 FPS).}
\label{tab:jetson}
\small
\begin{tabular}{lccc}
\toprule
...
\midrule
Training FPS & 17.5 & 1.87 & 9.4$\times$ \\
Render FPS & 316.9 & 38.7 & 8.2$\times$ \\
...
\textbf{Key Finding:} Training at 1.87 FPS is impractical for real-time operation, but rendering at 38.7 FPS significantly exceeds real-time requirements. This motivates a \textbf{train-on-desktop, deploy-on-edge} paradigm: reconstruct scenes on powerful workstations, then deploy trained models to edge devices for real-time rendering and navigation.
...
\begin{table}[t]
\centering
\caption{\textbf{Cross-Dataset Results.} GraphDeco generalizes well to Replica synthetic scenes, achieving higher PSNR due to noise-free ground truth.}
...
fr3\_office & 20.78 & 0.762 & 27,464 \\
\midrule
...
We ablate key hyperparameters using GraphDeco on TUM fr1\_desk:

\begin{table}[t]
\centering
\caption{\textbf{Densification Ablation.} Default parameters ($\tau_g=0.0002$, $\tau_\alpha=0.005$) provide optimal quality-efficiency balance.}
\label{tab:ablation}
\small
\begin{tabular}{cccc}
\toprule
$\tau_g$ & $\tau_\alpha$ & PSNR$\uparrow$ & Gaussians \\
\midrule
0.0002 & 0.005 & \textbf{15.85} & 51,565 \\
0.0001 & 0.005 & 15.92 & 78,340 \\
0.0004 & 0.005 & 15.41 & 34,220 \\
0.0002 & 0.01 & 15.23 & 42,100 \\
\bottomrule
\end{tabular}
\end{table}

Lower gradient threshold ($\tau_g = 0.0001$) increases Gaussian count by 52\% with only marginal quality improvement (+0.07 dB). Higher opacity threshold ($\tau_\alpha = 0.01$) aggressively prunes, reducing quality. The default parameters represent a reasonable Pareto point.

\subsection{Iteration Count Ablation}

We study convergence by evaluating quality at different training iterations:

\begin{table}[t]
\centering
\caption{\textbf{Training Iteration Ablation.} Quality saturates around 20K iterations; 30K provides marginal improvement.}
\label{tab:iteration_ablation}
\small
\begin{tabular}{lccc}
\toprule
Iterations & PSNR$\uparrow$ & SSIM$\uparrow$ & Time (s) \\
\midrule
5,000 & 12.34 & 0.512 & 45 \\
10,000 & 14.21 & 0.578 & 89 \\
20,000 & 15.52 & 0.621 & 178 \\
30,000 & 15.85 & 0.635 & 267 \\
50,000 & 15.91 & 0.638 & 445 \\
\bottomrule
\end{tabular}
\end{table}

Quality improves rapidly in early iterations (5K$\rightarrow$10K: +1.87 dB) but saturates beyond 20K iterations (20K$\rightarrow$50K: +0.39 dB). For real-time applications, 10K iterations provide 90\% of final quality at 33\% of training time.

\subsection{Learning Rate Schedule Analysis}

We compare learning rate schedules for position optimization:

\textbf{Exponential decay} (default): LR decays from $1.6 \times 10^{-4}$ to $1.6 \times 10^{-6}$ over 30K iterations. Achieves 15.85 dB PSNR with smooth convergence.

\textbf{Constant LR}: Fixed at $1.6 \times 10^{-4}$. Achieves only 14.92 dB due to oscillation in later iterations.

\textbf{Cosine annealing}: Achieves 15.78 dB with faster initial convergence but slightly worse final quality.

The exponential schedule provides optimal balance between fast initial convergence and stable refinement.
...
\textbf{(3) gsplat achieves best speed-quality trade-off.} Among evaluated engines, gsplat achieves competitive quality (15.0 dB PSNR) at the highest training speed (47.9 FPS), making it the recommended choice for real-time applications.

\textbf{(4) Edge deployment requires hybrid strategies.} While 3DGS training is impractical on edge hardware (1.87 FPS), rendering achieves real-time operation (38.7 FPS) with 6$\times$ power savings. This enables train-on-desktop, deploy-on-edge workflows for practical drone applications.

\subsection{Author Contributions}

This work represents a collaborative effort between two researchers with complementary expertise:

\textbf{Parsa Rezaei} designed the AirSplatMap evaluation framework and benchmark infrastructure, implemented the pose and depth estimation pipelines, developed the interactive visualization dashboard and results viewer, and established the experimental methodology for degradation analysis.

\textbf{Sunny Yoshimitsu Nguyen} implemented all five 3DGS engine integrations (GraphDeco, gsplat, MonoGS, SplaTAM, Gaussian-SLAM), developed the depth-based Gaussian initialization module, configured the training pipelines for each engine, conducted the engine comparison and edge deployment experiments, and contributed to the training dynamics analysis.
