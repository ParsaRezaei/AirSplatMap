...
\subsection{3DGS Optimization Objective}
...
\caption{\textbf{Evaluation Framework Overview.} We systematically evaluate all combinations across 17 sequences, 15+ methods, and 8 metrics totaling 3,991 benchmark runs.}
\label{fig:eval_framework}
\end{figure}

Unless otherwise noted, downstream 3DGS reconstruction uses \textbf{gsplat} as the state-of-the-art baseline engine; it achieves 15.0 dB PSNR on TUM fr1\_desk and 28.1 dB on Replica room0, making it the strongest renderer in our pool for comparing upstream pose/depth choices.

\subsection{Datasets}

We evaluate on established RGB-D benchmarks providing ground truth camera poses and depth:

\textbf{Data usage.} All frames are used for evaluation (no train/val/test split) because we analyze end-to-end error propagation rather than train a model. TUM RGB-D contributes seven sequences ($\sim$12K frames total) at 640$\times$480; 7Scenes~\cite{shotton20137scenes} contributes two sequences ($\sim$6K frames); Replica adds eight synthetic sequences ($\sim$12K frames) at 640$\times$480. Inputs are RGB; outputs are dense depth and rendered RGB for PSNR/SSIM/LPIPS evaluation.
...
\item \texttt{fr1\_xyz}, \texttt{fr2\_xyz} (798/3,669 frames): Simple translation patterns
...
\begin{itemize}
\item \texttt{chess} (4,000 frames): Tabletop with chess board---exhibits repetitive textures
\item \texttt{fire} (2,000 frames): Fireplace scene with varied textures
\end{itemize}
...
\begin{itemize}
\item Essential matrix estimation: OpenCV RANSAC with confidence 0.999, threshold 1.0 pixel
\item Feature extraction: 2000 keypoints maximum per frame
\item Flow methods: Farneb\"ack algorithm with 5 pyramid levels, 21-pixel window
...
\begin{itemize}
\item \textbf{Ubuntu Desktop:} Intel i5-9600K, 64GB RAM, NVIDIA RTX 2080 Ti (11GB), CUDA 12.1
...
\item \textbf{MacBook Pro:} Intel Core i9-9880H, 32 GB RAM, AMD Radeon Pro 5500M (pose/depth estimation only; 3DGS engines require CUDA)
\end{itemize}
...
\begin{itemize}
\item \textbf{ORB on 7Scenes chess:} Checkerboard corners produce high detector response but ambiguous descriptors, causing systematic mismatches that flip correspondences diagonally.
...
\begin{table}[t]
\centering
\caption{\textbf{Per-Sequence ATE (meters)} on selected sequences. Performance varies by scene characteristics. Bold indicates best for each sequence.}
...
fr3\_long\_office & 0.112 & 0.185 & \textbf{0.098} & SP \\
\midrule
\multicolumn{5}{c}{\textit{7Scenes}} \\
chess & \textbf{0.045} & 0.234 & 0.089 & Flow \\
\midrule
...
\textbf{Complex scenes (fr1\_room, fr3\_long\_office):} Larger gaps between methods. Long trajectories accumulate drift differently.

\textbf{Repetitive textures (7Scenes chess):} Dramatic difference---flow (0.045m) vs ORB (0.234m). Feature matching fails on checkerboard patterns.
...
\textbf{Acknowledgment:} The 3DGS reconstruction backend used for downstream quality analysis (Section~\ref{sec:downstream}) was implemented by Sunny Yoshimitsu Nguyen, who integrated five Gaussian splatting engines (GraphDeco, gsplat, MonoGS, SplaTAM, Gaussian-SLAM).
