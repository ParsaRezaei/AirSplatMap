...
\textbf{Stage 3: Reconstruction.} Poses and depths feed into GraphDeco~\cite{kerbl3Dgaussians} 3DGS with fixed hyperparameters, enabling controlled comparison of upstream method choices.
...
Experiments run on Ubuntu 22.04 with CUDA 12.1. Desktop: Intel i7-9700K, 64GB RAM, RTX 2080 Ti (11GB). Edge: NVIDIA Jetson AGX Orin at 35W TDP.
...
\caption{\textbf{Pose Estimation Comparison.} ATE RMSE (meters) across eleven methods. Classical methods (blue) achieve competitive accuracy at higher speeds than learned methods (orange).}
...
This work was completed by Parsa Rezaei, who designed the unified evaluation framework, implemented all eleven pose estimation methods and four depth estimators with consistent interfaces, conducted all 3,991 benchmark experiments across two hardware platforms, performed the statistical analysis, and wrote this paper.
